...
class GoogleChatNotify(_PluginBase):
    ...
    plugin_desc = "接收消息通知并转发至GoogleChat"
    ...
    def init_plugin(self, config: dict = None):
        ...
        if self._enabled and self._onlyonce:
            ...
            self._do_send(
                title="GoogleChat 排版测试",
                text="✅ 这是一个带内容的测试，标题下紧跟内容，无多余空行。"
            )
    ...
    def _do_send(self, title: str, text: str) -> schemas.Response:
        ...
        try:
            ...
            # 2. 内容处理：判断是否为空或为字符串 "None"
            if text and str(text).strip().lower() != "none":
                # 去掉中间的空行，直接 \n 换行
                full_content += f"\n{text}"
            ...
            logger.error(f"[GoogleChat] 推送异常: {str(e)}")
                ...
                'content': [
                    {
                        ...
                        'content': [
                            ...
                            {'component': 'VCol', 'props': {'cols': 12, 'md': 6}, 'content': [{'component': 'VSwitch', 'props': {'model': 'onlyonce', 'label': '保存并测试'}}]}
                        ]
                    },
                    {
                        'component': 'VRow',
                        'content': [
                            {'component': 'VCol', 'props': {'cols': 12}, 'content': [{'component': 'VTextField', 'props': {'model': 'google_chat_url', 'label': 'Webhook URL', 'placeholder': 'https://...', 'clearable': True}}]}
                        ]
                    },
                    {
                        'component': 'VRow',
                        'content': [
                            {'component': 'VCol', 'props': {'cols': 12}, 'content': [{'component': 'VSelect', 'props': {'multiple': True, 'chips': True, 'model': 'msgtypes', 'label': '消息类型', 'items': MsgTypeOptions}}]}
                        ]
                    }
                ]
            }
    ...
    @staticmethod
    def get_command() -> List[Dict[str, Any]]: pass
